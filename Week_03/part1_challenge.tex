...
\bottomnote{Real company data: Slack processes 2.5M messages daily, Airbnb 1M reviews monthly}
\end{frame}
...
\bottomnote{Technical term we'll build to: ``Contextual Word Embeddings'' - but first, let's understand the problem}
\end{frame}
...
\bottomnote{This multi-layer challenge is why human annotators only agree 79\% of the time on sentiment}
\end{frame}
...
\bottomnote{This is why Fortune 500 companies invest millions in NLP - the scale makes human analysis impossible}
\end{frame}
...
\bottomnote{Shannon's theorem: Lossless compression impossible beyond entropy limit - emotion has high entropy}
\end{frame}