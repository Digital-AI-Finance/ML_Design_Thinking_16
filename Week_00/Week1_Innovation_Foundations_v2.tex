...
\begin{frame}[t]{ML-Enhanced Design Process}
\centering
\includegraphics[width=0.9\textwidth]{figures/design_ml_integration.pdf}
\vfill
\footnotesize
...
\vfill
\footnotesize
\textbf{Impact:} ML-assisted innovation generates 100x more ideas. Success rate improves 7x. Time reduced by 90\%. Full integration maximizes all metrics.
\end{frame}

\begin{frame}[t]{Model Learning Curves}
\centering
\includegraphics[width=0.85\textwidth]{figures/learning_curves.pdf}
\vfill
\footnotesize
\textbf{Convergence:} Training and validation scores converge indicating good generalization. No overfitting observed. Model ready for deployment.
\end{frame}
...
\vfill
\footnotesize
\textbf{Problem Evolution:} Week 1 success generates 5000+ ideas. Too many to handle manually. Need systematic organization. Week 2 will introduce K-Means clustering solution.
\end{frame}
...
\vfill
\footnotesize
\textbf{Preview:} Week 2 will teach K-Means clustering algorithm to bring order to chaos.
\end{frame}
...
\centering
\begin{beamercolorbox}[sep=8pt,center]{title}
\usebeamerfont{title}\Large Week 1 Summary\par
\end{beamercolorbox}
\vfill
\end{frame}
...
\vspace{10pt}
\textbf{Problem Chain:}\\
\small Success $\rightarrow$ Too many ideas $\rightarrow$ Need organization (Week 2)
\end{column}
\end{columns}
\end{frame}

\begin{frame}[t]{Learning Objectives Achieved}
\begin{table}
\centering
\small
\begin{tabular}{ll}
