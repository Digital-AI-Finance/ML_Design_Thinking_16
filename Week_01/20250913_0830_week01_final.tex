...
\item \textbf{Apply} the elbow method
\item \textbf{Calculate} distance metrics
\item \textbf{Evaluate} cluster quality
\item \textbf{Compare} different algorithms
\item \textbf{Select} appropriate methods
\end{itemize}
\end{tcolorbox}
\end{column}

\begin{column}{0.48\textwidth}
\begin{tcolorbox}[colback=mlorange!10, colframe=mlorange!50, title=Hands-on Skills]
\normalsize
\begin{itemize}
\item Code K-means from scratch
\item Interpret silhouette scores
\item Choose between algorithms
\item Tune parameters effectively
\item Handle different data shapes
\item Preprocess data correctly
\end{itemize}
\end{tcolorbox}
\end{column}
\end{columns}
...
\framesubtitle{Innovation Applications You'll Master}

\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{tcolorbox}[colback=mlorange!10, colframe=mlorange!50, title={By the end of Part 3, you will:}]
\normalsize
\begin{itemize}
