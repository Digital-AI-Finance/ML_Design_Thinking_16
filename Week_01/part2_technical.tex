...
\begin{tcolorbox}[colback=mlred!10, colframe=mlred!50, title=The Fragmentation Crisis]
\normalsize
\begin{itemize}
\item \textbf{70\%} of innovations fail due to misalignment
\item \textbf{85\%} miss their target audience
\item \textbf{60\%} duplicate existing solutions
\end{itemize}
\end{tcolorbox}

\vspace{0.3em}
\textcolor{mlpurple}{\textbf{Why This Happens:}}
\begin{itemize}
\small
\item Treating all ideas the same
\item Missing subtle patterns
\item No systematic categorization
\item Human cognitive limits
\end{itemize}
\end{column}
\end{columns}
...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{\large For Innovations, What Features Matter?}
\vspace{0.3em}
...
\begin{itemize}
\small
\item Points become equally distant
\item Everything is on the edge
\item Volume concentrates at boundaries
\item Traditional intuition fails
\end{itemize}
\end{column}
...
\begin{center}
\includegraphics[width=0.75\textwidth]{charts/dendrogram_example.pdf}
\end{center}
...
% NEW: Data Preprocessing Pipeline with Example Data
\begin{frame}
\frametitle{\Large Data Preprocessing Pipeline - Example}
\framesubtitle{Real Innovation Data Transformation}
