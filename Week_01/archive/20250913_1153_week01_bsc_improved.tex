...
\midrule
\textcolor{mlpurple}{Discover} & 1-2 & Find hidden innovation opportunities \\
...
\vspace{0.5em}
\begin{center}
\Large\textcolor{mlpurple}{\textbf{This Week: Clustering for Innovation Pattern Discovery}}
\end{center}
\end{frame}
...
\framesubtitle{Quick Knowledge Check}
...
\normalsize
\begin{itemize}
\item Natural groupings (like sorting laundry by color)
\item Similar approaches (things that work the same way)
\item Hidden patterns (connections you didn't see before)
\item Innovation relationships (which ideas go together)
\end{itemize}
...
\begin{frame}
\frametitle{\Large K-Means: The Basic Clustering Method}
\framesubtitle{Like Finding Neighborhoods in a City}
...
\normalsize
\begin{enumerate}
\item Choose K \textit{(number of clusters)}
\item Place K random centroids \textit{(initial group centers)}
\item Assign points to nearest centroid \textit{(by calculating distances)}
\item Update centroids \textit{(move to cluster mean)}
\item Repeat until convergence \textit{(no changes occur)}
\end{enumerate}
...
\begin{center}
\textbf{Euclidean}\\
\normalsize "As the crow flies" distance\\
...
\begin{center}
\textbf{Manhattan}\\
\normalsize Walking distance in a city\\
...
\begin{center}
\textbf{Cosine}\\
\normalsize Direction similarity\\
...
\normalsize
\begin{itemize}
\item Ranges from -1 to +1 \textit{(mathematical measure)}
\item Higher = better separation \textit{(clearer groups)}
\item Our score: \textbf{0.73} \textit{(good clustering!)}
\end{itemize}
...
\begin{frame}
\frametitle{\Large Comparing Evaluation Metrics}
\framesubtitle{Different Metrics for Different Data Patterns}

\begin{center}
\includegraphics[width=0.95\textwidth]{charts/evaluation_metrics_comparison.pdf}
\end{center}
\end{frame}
...
\begin{frame}
\frametitle{\Large Choosing the Right Algorithm}
\framesubtitle{Comparison of Clustering Methods}
...
\begin{center}
\includegraphics[width=0.95\textwidth]{charts/algorithm_visual_examples.pdf}
\end{center}
\end{frame}
...
\begin{center}
\includegraphics[width=0.95\textwidth]{charts/parameter_tuning_guide.pdf}
\end{center}
\end{frame}
...
% Slide 20: AI-Generated Innovation Archetypes
\begin{frame}
...
% NEW: Innovation Taxonomy
\begin{frame}
\frametitle{\Large Innovation Taxonomy Framework}
\framesubtitle{Types, Relationships, and Impact Levels}
...
% Slide 21: Innovation Pattern Maps
\begin{frame}
\frametitle{\Large Innovation Pattern Mapping by Cluster}
\framesubtitle{Understanding Each Category's Impact}

\begin{center}
\includegraphics[width=0.85\textwidth]{charts/innovation_pattern_maps.pdf}
\end{center}
\end{frame}

% NEW: Innovation Lifecycle
\begin{frame}
\frametitle{\Large Innovation Lifecycle Stages}
\framesubtitle{How Clustering Reveals Innovation Patterns at Each Stage}
...
% NEW: Innovation Ecosystem Mapping
\begin{frame}
\frametitle{\Large Innovation Ecosystem Mapping}
\framesubtitle{From Complex Networks to Clear Clusters}
...
\framesubtitle{Application Knowledge Check}
...
\begin{center}
\includegraphics[width=0.95\textwidth]{charts/tsne_pca_comparison.pdf}
\end{center}
\end{frame}
