...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Organizational culture determines execution success - systematic experimentation requires embedded practices not mandated procedures}
\end{frame}
...
\includegraphics[width=0.85\textwidth]{charts/metric_tree_example.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Metric selection determines optimization direction - specificity, measurability, and value alignment constrain what systems can improve}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Audience context shapes information presentation - identical data requires different framing for different stakeholders}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Statistical literacy compounds across iterations - understanding accumulates faster than individual technique mastery}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/simpsons_paradox.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Aggregation can reverse segment-level conclusions - compositional differences distort summary statistics when subgroups are imbalanced}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/experiment_decision_matrix.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Decision criteria reduce deliberation overhead - pre-specified thresholds eliminate recursive argumentation}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Experimental effects evolve post-intervention - immediate results diverge from long-term outcomes as novelty effects decay}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Ethical practice transcends regulatory compliance - user respect builds sustainable trust beyond legal requirements}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Innovation velocity emerges from multiplicative factors - experiment frequency, success rate, and effect magnitude compound together}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Infrastructure investment scales with organizational complexity - managed services enable early exploration while custom systems optimize mature operations}
\end{frame}

% Slide 10: When to Use A/B Testing - Judgment Criteria
\begin{frame}[t]{When to Use A/B Testing: Judgment Criteria}
\vspace{-0.5cm}
\begin{center}
\includegraphics[width=0.75\textwidth]{charts/validation_method_decision.pdf}
\end{center}

\begin{center}
\textcolor{mlpurple}{\textbf{The Principle:}} Match validation method to traffic, stakes, and reversibility - A/B testing for safe, measurable, reversible decisions
\end{center}

\vspace{\fill}
\footnotesize\textcolor{mlgray}{Judgment criteria enable wise method selection - A/B testing excels when you have traffic, time, and measurable outcomes}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Conceptual understanding enables practical application - theoretical frameworks guide implementation decisions}
\end{frame}