...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Hypothesis pre-registration prevents post-hoc rationalization - documenting predictions before observing results ensures scientific integrity}
\end{frame}
...
\includegraphics[width=0.85\textwidth]{charts/statistical_power_curves.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Sample size determines statistical power - insufficient observations lead to inconclusive results regardless of analytical rigor}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Randomization strategy adapts to threat structure - different confounding patterns require different allocation mechanisms}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Effect size and confidence intervals quantify practical significance - p-values alone obscure magnitude and precision}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/type_i_ii_errors.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Statistical error types trade off against each other - reducing false positives increases false negatives without larger samples}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/bayesian_ab_posterior.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Bayesian approaches gain adoption where uncertainty quantification matters - probabilistic frameworks enable principled decision-making under incomplete information}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/confidence_intervals.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Repeated sampling visualization clarifies confidence interval interpretation - seeing multiple intervals reveals the procedure's long-run behavior}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/sequential_testing_boundaries.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Planned interim analyses preserve statistical validity - alpha spending functions distribute error control across observation points}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/thompson_sampling_demo.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Exploration-exploitation algorithms prioritize optimization over inference - adaptive allocation minimizes opportunity cost during learning}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/multi_armed_bandit_exploration.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Adaptive allocation reduces opportunity cost during experimentation - shifting traffic toward better variants minimizes regret compared to fixed splits}
\end{frame}
...
\begin{columns}[T]
\column{0.55\textwidth}
\includegraphics[width=0.85\textwidth]{charts/causal_inference_dag.pdf}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Randomized controlled trials establish causal claims - random assignment eliminates confounding through design rather than statistical adjustment}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Multi-objective optimization reflects practical reality - real systems rarely optimize single metrics in isolation}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Experimental method selection requires context matching - traffic volume, risk tolerance, and decision urgency constrain algorithmic choices}
\end{frame}