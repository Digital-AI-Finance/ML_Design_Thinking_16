...
\vspace{\fill}
\small\textcolor{gray}{Supervised learning transforms labeled examples into predictive models - regression predicts continuous values from multiple correlated features}
\end{frame}
...
\vspace{\fill}
\small\textcolor{gray}{Linear regression assumes additive relationships - optimal under Gauss-Markov conditions but restrictive for real-world complexity}
\end{frame}
...
\vspace{\fill}
\small\textcolor{gray}{Problem formulation determines algorithm choice - continuous targets require regression, discrete categories need classification with appropriate loss functions}
\end{frame}
...
\item ``Hughes phenomenon'' (Hughes 1968): Performance degrades as features increase beyond sample capacity
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
...
\vspace{\fill}
\small\textcolor{gray}{Curse of dimensionality: volume grows exponentially while data remains sparse - distances become uninformative and nearest neighbors lose meaning (Bellman 1961)}
\end{frame}
...
\vspace{\fill}
\small\textcolor{gray}{Combinatorial explosion creates $2^n$ possible interactions - regularization and feature selection become necessary to prevent overfitting in high dimensions}
\end{frame}