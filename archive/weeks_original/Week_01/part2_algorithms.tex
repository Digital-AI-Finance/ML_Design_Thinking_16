...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Clustering is unsupervised learning - algorithms find patterns without labeled examples or predefined categories}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{K-means initialization matters - K-means++ spreads centers intelligently to avoid poor local optima}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Lloyd's algorithm minimizes within-cluster variance iteratively - convergence guaranteed but solution depends on initialization}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Choosing K balances granularity and actionability - too few clusters miss patterns, too many create analysis paralysis}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Elbow method quantifies trade-off between cluster count and within-cluster variance - look for diminishing returns}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Distance metric choice affects cluster quality - match the metric to your data type and domain requirements}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Silhouette score validates cluster quality by measuring cohesion vs separation - essential for comparing different K values}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Density-based clustering handles arbitrary shapes and outliers - connectivity definitions enable non-convex cluster discovery}
\end{frame}
...
\vspace{\fill}
\footnotesize\textcolor{mlgray}{Algorithm selection framework: start simple (K-means), upgrade only when data characteristics demand it (shapes, outliers, unknown K)}
\end{frame}

% Slide: When to Use Which Algorithm - Judgment Criteria
\begin{frame}[t]{When to Use Which Clustering Algorithm: Judgment Criteria}
\vspace{-0.5cm}
\begin{center}
\includegraphics[width=0.75\textwidth]{charts/clustering_algorithm_decision.pdf}
\end{center}

\begin{center}
\textcolor{mlpurple}{\textbf{The Principle:}} Start simple (K-means), upgrade if needed (DBSCAN for shapes, Hierarchical for structure)
\end{center}

\vspace{\fill}
\footnotesize\textcolor{mlgray}{Judgment criteria enable wise algorithm selection - avoid over-engineering simple problems or under-powering complex ones}
\end{frame}